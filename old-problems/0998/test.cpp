...
TEST_CASE("998. Maximum Binary Tree II") {
  SECTION("Example 1") {
    TreeNode* root{toTreeNode({4, 1, 3, null, null, 2})};
    TreeNode* actual{Solution{}.insertIntoMaxTree(root, 5)};
    std::vector<std::optional<int>> expected{5, 4, null, 1, 3, null, null, 2};
    REQUIRE(fromTreeNode(actual) == expected);
  }

  SECTION("Example 2") {
    TreeNode* root{toTreeNode({5, 2, 4, null, 1})};
    TreeNode* actual{Solution{}.insertIntoMaxTree(root, 3)};
    std::vector<std::optional<int>> expected{5, 2, 4, null, 1, null, 3};
    REQUIRE(fromTreeNode(actual) == expected);
  }

  SECTION("Example 3") {
    TreeNode* root{toTreeNode({5, 2, 3, null, 1})};
    TreeNode* actual{Solution{}.insertIntoMaxTree(root, 4)};
    std::vector<std::optional<int>> expected{5, 2, 4, null, 1, 3};
    REQUIRE(fromTreeNode(actual) == expected);
  }
}
