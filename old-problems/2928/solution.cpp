...
class Solution {
 public:
  int distributeCandies(int n, int limit) {
    ...
    const int oneLimit{excludeOne > 1 ? 3 * excludeOne * (excludeOne - 1) / 2 : 0};

    const int excludeTwo{n - 2 * (limit + 1) + 2};
    const int twoLimit{excludeTwo > 1 ? 3 * excludeTwo * (excludeTwo - 1) / 2 : 0};

    const int excludeThree{n - 3 * (limit + 1) + 2};
    const int threeLimit{excludeThree > 1 ? excludeThree * (excludeThree - 1) / 2 : 0};
