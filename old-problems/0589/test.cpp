...
TEST_CASE("589. N-ary Tree Preorder Traversal") {
  SECTION("Example 1") {
    Node* root{toNode({1, null, 3, 2, 4, null, 5, 6})};
    std::vector<int> expected{1, 3, 5, 6, 2, 4};
    REQUIRE(Solution{}.preorder(root) == expected);
  }

  SECTION("Example 2") {
    Node* root{toNode({1, null, 2, 3, 4, 5, null, null, 6, 7, null, 8, null, 9, 10, null, null,
